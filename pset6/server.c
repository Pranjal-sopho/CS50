...
char* indexes(const char* path)
{
    // TODO
        ...
        char* inx = malloc(sizeof(strlen(path) + strlen("/index.php") + 1));
        strcpy(inx, path);
        strcat(inx, "/index.html");

        if (access(inx, F_OK))
        {
            inx[strlen(path) + strlen("/index.html")] = '\0';
             return inx;
        }

        strcpy(inx, path);
        strcat(inx, "/index.php");

        if (access(inx, F_OK))
        {
            inx[strlen(path) + strlen("/index.php")] = '\0';
           return inx;
        }
